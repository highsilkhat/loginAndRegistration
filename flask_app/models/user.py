...
class User:
    ...
    def create_user(cls, data):
        ...
        newUserId = connectToMySQL('loginAndRegistrationSchema').query_db(query, data)

        return newUserId

    @classmethod
    def getOneUserByEmail(cls, data):

        query = 'SELECT * FROM users WHERE email = %(email)s;'

        results = connectToMySQL('loginAndRegistrationSchema').query_db(query, data)
    ...
    def validate_user(data):
        ...
        if len(User.getOneUserByEmail({'email' :data['email']})) > 0:
            is_valid = False
